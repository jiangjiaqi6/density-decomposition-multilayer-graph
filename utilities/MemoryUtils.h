...
double getPeakRSSNew(Clock &allClock) {
	...
	for (std::string line; std::getline(file, line);) {
		if (line.rfind("VmPeak:", 0) == 0) {
            ...
            mem = std::stod(line.substr(7)) / 1024.0;
...
long double GetPeakRSSInMB() {
    return getPeakRSS() / 1024.0L / 1024;
}


long double GetCurrentRSSInMB() {
    return getCurrentRSS() / 1024.0L / 1024;
}


#endif //MLCDEC_MEMORYUTILS_H
